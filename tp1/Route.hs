...
data RouteException = EmptyRouteException 
                    | InvalidCityException String
                    deriving (Show)
...
newR :: [ String ] -> Route   
newR [] = throw EmptyRouteException
...
inOrderR (Rou (c:cs)) city1 city2
        | not (inRouteR (Rou (c:cs)) city2 ) = throw (InvalidCityException city2)  -- Si no encuentro city2 en la lista de ciudades, tiro excepción
